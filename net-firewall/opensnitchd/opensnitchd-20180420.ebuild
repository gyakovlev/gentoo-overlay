...
MY_PN="${PN::-1}"
ARCHIVE_URI="https://${EGO_PN}/archive/${EGIT_COMMIT}.tar.gz -> ${MY_PN}-${PV}.tar.gz"
...
# runs sudo pip install
RESTRICT="test"

src_compile() {
	cd src/${EGO_PN}/daemon || die
	GOPATH="${S}" go build -o opensnitchd -v . || die
}

src_install() {
	dodoc src/${EGO_PN}/README.md
	dobin src/${EGO_PN}/daemon/opensnitchd
	systemd_dounit src/${EGO_PN}/daemon/opensnitchd.service
	newinitd "${FILESDIR}"/opensnitchd.initd opensnitchd
	newconfd "${FILESDIR}"/opensnitchd.confd opensnitchd
